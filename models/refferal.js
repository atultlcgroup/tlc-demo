...
let getRefferalData2=  (data)=>{
    ...
    return new Promise(async(resolve,reject)=>{
        try{
           ...
           console.log(`select referral_code__c,member_id__c,membership_number__C from tlcsalesforce.account p1 inner join tlcsalesforce.membership__c p2 on p1.member_id__c = p2.member__r__member_id__C where p2.membership_number__C='${data.membershipNumber}'`) 
           
           let result= await pool.query(`select referral_code__c,member_id__c,membership_number__C from tlcsalesforce.account p1 left join tlcsalesforce.membership__c p2 on p1.member_id__c = p2.member__r__member_id__C where p2.membership_number__C='${data.membershipNumber}'`)
           
           console.log("under the resolve promise",result);
            resolve(result)
