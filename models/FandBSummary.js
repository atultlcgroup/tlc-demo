...
let insertSummarizedDataToFandB=async(netSpentAPCTCovers,coversInDBL,coversOfDayInCurrentMonth,outlet_id , i)=>{
    return new Promise(async(resolve,reject)=>{
        try{
            ...
            if(data.length ==0){
            await pool.query(`INSERT INTO tlcsalesforce.f_b_summary__c(Month__c,Year__c,Monthly_Total_Cover__c,
            ...
            covers_for_wed__c= '${coversOfDayInCurrentMonth['Wednesday']}', net_spends__c='${netSpentAPCTCovers['totalamount']}', covers_by_time_in_a_day_for_breakfast__c ='${coversInDBL['covers_in_breakfaset']}', covers_by_time_in_a_day_for_dinner__c = '${coversInDBL['covers_in_dinner']}', covers_for_mon__c = '${coversOfDayInCurrentMonth['Monday']}', covers_for_sun__c ='${coversOfDayInCurrentMonth['Sunday']}', apc_month_wise__c = '${netSpentAPCTCovers['apc']}', outlet__c= '${outlet_id}', covers_for_sat__c = '${coversOfDayInCurrentMonth['Saturday']}', covers_for_tue__c = '${coversOfDayInCurrentMonth['Tuesday']}', covers_by_time_in_a_day_for_lunch__c ='${coversInDBL['covers_in_lunch']}', covers_for_thr__c = '${coversOfDayInCurrentMonth['Thursday']}', covers_for_fri__c='${coversOfDayInCurrentMonth['Friday']}' where date_part('year', createddate)= date_part('year',CURRENT_DATE - INTERVAL '${i} MONTH') 
            and date_part('month', createddate) = date_part('month',CURRENT_DATE - INTERVAL '${i} MONTH') and outlet__c='${outlet_id}'`);
