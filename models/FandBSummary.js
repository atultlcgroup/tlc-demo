...
let findNetSpentAPCTCovers = (outlet_id , i)=>{
    return new Promise(async(resolve,reject)=>{
        try{
          ...
          and date_part('month', actual_bill_date__c) = date_part('month',CURRENT_DATE - INTERVAL '${i} MONTH') AND actual_bill_date__c<=CURRENT_DATE - INTERVAL '${i} MONTH' AND outlet__c='${outlet_id}'`)
...
let totalCoversInDBL = (outlet_id , i)=>{
    return new Promise(async(resolve,reject)=>{
    try{
    ...
    //    and date_part('month', actual_bill_date__c) = date_part('month',CURRENT_DATE) AND actual_bill_date__c<=CURRENT_DATE AND outlet__c='${outlet_id}'`) 
       let totalCoverForBreakFast = await pool.query(`select sum(cast(pos_cheque_details__c.covers__c AS DOUBLE PRECISION)) covers_in_breakfaset from tlcsalesforce.pos_cheque_details__c where length(bill_time__c)>0 and bill_time__c::time <= '${BREAKFASTTIME[1]}'::time and bill_time__c::time >= '${BREAKFASTTIME[0]}'::time and length(covers__c)>0  and date_part('year', actual_bill_date__c)= date_part('year',CURRENT_DATE - INTERVAL '${i} MONTH') 
    and date_part('month', actual_bill_date__c) = date_part('month',CURRENT_DATE - INTERVAL '${i} MONTH') AND actual_bill_date__c<=CURRENT_DATE - INTERVAL '${i} MONTH' AND outlet__c='${outlet_id}'`)
    const resultcovers_in_breakfaset = totalCoverForBreakFast ? totalCoverForBreakFast.rows : [{covers_in_breakfaset: 0}]
    let totalCoverForLunch = await pool.query(`select  sum(cast(pos_cheque_details__c.covers__c AS DOUBLE PRECISION)) covers_in_lunch from tlcsalesforce.pos_cheque_details__c where length(bill_time__c)>0 and bill_time__c::time > '${LUNCHTIME[0]}'::time and bill_time__c::time <= '${LUNCHTIME[1]}'::time   and length(covers__c)>0 and  date_part('year', actual_bill_date__c)= date_part('year',CURRENT_DATE - INTERVAL '${i} MONTH') 
    and date_part('month', actual_bill_date__c) = date_part('month',CURRENT_DATE - INTERVAL '${i} MONTH') AND actual_bill_date__c<=CURRENT_DATE - INTERVAL '${i} MONTH' AND outlet__c='${outlet_id}' `)
    const resulttotalCoverForLunch = totalCoverForLunch ? totalCoverForLunch.rows : [{covers_in_lunch: 0}]
    let totalCoverForDinner = await pool.query(`select sum(cast(pos_cheque_details__c.covers__c AS DOUBLE PRECISION)) covers_in_dinner from tlcsalesforce.pos_cheque_details__c where length(bill_time__c)>0 and (bill_time__c::time  >'${DINNERTIME[0]}'::time or bill_time__c::time < '${DINNERTIME[1]}'::time)  and length(covers__c)>0 and date_part('year', actual_bill_date__c)= date_part('year',CURRENT_DATE - INTERVAL '${i} MONTH') 
    and date_part('month', actual_bill_date__c) = date_part('month',CURRENT_DATE - INTERVAL '${i} MONTH') AND actual_bill_date__c<=CURRENT_DATE - INTERVAL '${i} MONTH' AND outlet__c='${outlet_id}'`)
...
let coversDayWise= (outlet_id , i)=>{
    return new Promise(async(resolve,reject)=>{
        ...
        //  console.log(`select sum(cast(pos_cheque_details__c.covers__c AS DOUBLE PRECISION)),to_char(pos_cheque_details__c.bill_date__c, 'Day') AS "Day"
        //  from tlcsalesforce.pos_cheque_details__c where length(covers__c) >0 and date_part('year', actual_bill_date__c)= date_part('year',CURRENT_DATE - INTERVAL '${i} MONTH') 
        // and date_part('month', actual_bill_date__c) = date_part('month',CURRENT_DATE - INTERVAL '${i} MONTH') AND actual_bill_date__c<=CURRENT_DATE - INTERVAL '${i} MONTH' AND outlet__c='${outlet_id}' group by "Day"`)
        let  coversOfDayInCurrentMonth=await  pool.query(`select sum(cast(pos_cheque_details__c.covers__c AS DOUBLE PRECISION)),to_char(pos_cheque_details__c.bill_date__c, 'Day') AS "Day"
           from tlcsalesforce.pos_cheque_details__c where length(covers__c) >0 and date_part('year', actual_bill_date__c)= date_part('year',CURRENT_DATE - INTERVAL '${i} MONTH') 
          and date_part('month', actual_bill_date__c) = date_part('month',CURRENT_DATE - INTERVAL '${i} MONTH') AND actual_bill_date__c<=CURRENT_DATE - INTERVAL '${i} MONTH' AND outlet__c='${outlet_id}' group by "Day"`)
...
let insertSummarizedDataToFandB=async(netSpentAPCTCovers,coversInDBL,coversOfDayInCurrentMonth,outlet_id , i)=>{
    return new Promise(async(resolve,reject)=>{
        try{
            let checkOutletForMonth = await pool.query(`select id from tlcsalesforce.f_b_summary__c where  year__c= cast(date_part('year',CURRENT_DATE - INTERVAL '${i} MONTH') as varchar)
            and month__c = TRIM(cast(TO_CHAR(current_timestamp - INTERVAL '${i} MONTH','Month') as varchar)) and outlet__c='${outlet_id}'`)
   let data = checkOutletForMonth ? checkOutletForMonth.rows : [];
         if(data.length ==0){
            await pool.query(`INSERT INTO tlcsalesforce.f_b_summary__c( external_id__c,Month__c,Year__c,Monthly_Total_Cover__c,
            ...
            covers_for_wed__c= '${coversOfDayInCurrentMonth['Wednesday']}', net_spends__c='${netSpentAPCTCovers['totalamount']}', covers_by_time_in_a_day_for_breakfast__c ='${coversInDBL['covers_in_breakfaset']}', covers_by_time_in_a_day_for_dinner__c = '${coversInDBL['covers_in_dinner']}', covers_for_mon__c = '${coversOfDayInCurrentMonth['Monday']}', covers_for_sun__c ='${coversOfDayInCurrentMonth['Sunday']}', apc_month_wise__c = '${netSpentAPCTCovers['apc']}', outlet__c= '${outlet_id}', covers_for_sat__c = '${coversOfDayInCurrentMonth['Saturday']}', covers_for_tue__c = '${coversOfDayInCurrentMonth['Tuesday']}', covers_by_time_in_a_day_for_lunch__c ='${coversInDBL['covers_in_lunch']}', covers_for_thr__c = '${coversOfDayInCurrentMonth['Thursday']}', covers_for_fri__c='${coversOfDayInCurrentMonth['Friday']}' where   year__c= cast(date_part('year',CURRENT_DATE - INTERVAL '${i} MONTH') as varchar)
            and month__c = cast(date_part('month',CURRENT_DATE - INTERVAL '${i} MONTH') as varchar) and outlet__c='${outlet_id}'`);
...
let FandBSummaryReport = async () => {
    return new Promise(async(resolve,reject)=>{
        try {
            ...
            for(d of data){
                for(let i =0;i<APPLICABLE_MONTH_FOR_FNB;i++){

                    const netSpentAPCTCovers = await findNetSpentAPCTCovers(d.outlet_id , i)
                    const coversInDBL = await totalCoversInDBL(d.outlet_id , i)
                    const coversOfDayInCurrentMonth= await coversDayWise(d.outlet_id , i)
                    await insertSummarizedDataToFandB(netSpentAPCTCovers,coversInDBL,coversOfDayInCurrentMonth,d.outlet_id , i)
                        }
                }
            resolve(data)
        } catch (e) {
